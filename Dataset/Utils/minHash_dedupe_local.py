"""MinHash LSH deduplication for HuggingFace datasets.

Adapted from the BigCode project (https://huggingface.co/blog/dedup).
Computes MinHash signatures, performs Locality-Sensitive Hashing to find
candidate near-duplicate pairs, clusters them via Union-Find, and retains
only the representative document from each cluster.
"""
...
class UnionFind:
    """Disjoint-set (Union-Find) with path compression for clustering duplicate pairs."""
...
if __name__ == "__main__":
    ...
    def run(
        ...
        final_data.save_to_disk(str(output))
