...
class AceCodeRM(Qwen2ForCausalLM):
    """Qwen2 backbone with a ValueHead for scalar reward scoring (AceCode architecture)."""
    ...
    def forward(
        ...
        **kwargs,
        ...
        rm_scores = value.gather(
            dim=-1, index=(attention_mask.sum(dim=-1, keepdim=True).to(value.device) - 1)
